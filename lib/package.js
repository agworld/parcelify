...
Package.prototype.createAsset = function( thisAssetSrcPath, assetType ) {
	var thisAsset = new Asset( thisAssetSrcPath, assetType, this.assetTransformsByType[ assetType ] );
...
Package.prototype.createWatchers = function( assetTypes, packageTransform ) {
	this._createPackageJsonWatcher( assetTypes, packageTransform );
...
Package.prototype._createPackageJsonWatcher = function( assetTypes, packageTransform ) {
	var _this = this;
	...
	this.assetJsonWatcher.on( 'changed', function( srcPath ) {
		...
		fs.readFile( srcPath, 'utf8', function( err, packageJson ) {
			...
			if( packageTransform ) packageJson = packageTransform( packageJson, _this.path );

			Package.getOptionsFromPackageJson( _this.id, _this.path, packageJson, assetTypes, function( err, options ) {
				if( err ) return _this.emit( 'error', err );
